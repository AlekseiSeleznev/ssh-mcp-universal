...
SSH MCP Universal is a Model Context Protocol server that enables Claude Code to manage multiple SSH connections. It provides tools for executing commands, transferring files, and managing deployments across remote servers. The canonical MCP identifier is `ssh-universal`.
...
```bash
claude mcp add ssh-universal node /absolute/path/to/ssh-mcp-universal/src/index.js
```
