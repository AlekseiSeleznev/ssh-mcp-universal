      ...
      for (const line of lines) {
        ...
        if (entry) {
          ...
          fingerprints.push({
            host: entry.host,
            type: entry.keyType,
            fingerprint: computeKeyFingerprint(keyData),
            fullKey: line
          });
        }
      }
...
export function verifyKnownHostFingerprint(host, port = 22, key) {
  const currentKeys = getCurrentHostKey(host, port);
  if (!currentKeys || currentKeys.length === 0) {
    return {
      allowed: false,
      reason: 'not_in_known_hosts',
      presentedFingerprint: computeKeyFingerprint(key)
    };
  }

  const presentedFingerprint = computeKeyFingerprint(key);
  const matches = currentKeys.some((currentKey) => currentKey.fingerprint === presentedFingerprint);
  ...
  if (matches) {
    return {
      ...
      currentFingerprints: currentKeys.map((currentKey) => currentKey.fingerprint)
    };
  }
