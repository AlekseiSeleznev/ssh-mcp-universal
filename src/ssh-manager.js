...
import fs from 'fs';
import os from 'os';
import { addHostKey, verifyKnownHostFingerprint } from './ssh-key-manager.js';
...
class SSHManager {
  ...
  async connect(options = {}) {
    return new Promise((resolve, reject) => {
      this.lastHostVerificationError = null;
      ...
      this.client.on('error', (err) => {
        this.connected = false;
        if (this.lastHostVerificationError) {
          reject(this.lastHostVerificationError);
          return;
        }
        reject(err);
      });
      ...
      if (this.hostKeyVerification) {
        connConfig.hostVerifier = (hostKeyBuffer) => {
          const port = this.config.port || 22;
          const host = this.config.host;
          const verification = verifyKnownHostFingerprint(host, port, hostKeyBuffer);

          if (verification.allowed) {
            logger.info('Host key verified', {
              host,
              port,
              fingerprint: verification.presentedFingerprint
            });
            return true;
          }

          if (verification.reason === 'not_in_known_hosts' && this.autoAcceptHostKey) {
          ...
          if (verification.reason === 'key_mismatch') {
            this.lastHostVerificationError = new Error(
              `SSH host key mismatch for ${host}:${port}. Presented ${verification.presentedFingerprint}, expected one of: ${verification.currentFingerprints.join(', ')}. Use ssh_key_manage to verify and update the key.`
            );
            logger.warn('SSH host key mismatch', {
              host,
              port,
              presentedFingerprint: verification.presentedFingerprint,
              expectedFingerprints: verification.currentFingerprints
            });
            return false;
          }

          this.lastHostVerificationError = new Error(
            `SSH host ${host}:${port} is not trusted yet. Use ssh_key_manage with action "accept" or enable autoAcceptHostKey explicitly.`
          );
          logger.warn('Unknown SSH host rejected', {
            host,
            port,
            fingerprint: verification.presentedFingerprint
          });
  ...
  async execCommandStream(command, options = {}) {
    ...
    const { cwd, onStdout, onStderr, stdin = null } = options;
