...
let servers = {};
const dashboardTestResults = new Map();

function toRuntimeServerConfig(serverConfig) {
  return {
    ...serverConfig,
    keypath: serverConfig.keyPath || serverConfig.keypath,
    default_dir: serverConfig.defaultDir || serverConfig.default_dir,
    sudo_password: serverConfig.sudoPassword || serverConfig.sudo_password,
    proxy_jump: serverConfig.proxyJump || serverConfig.proxy_jump
  };
}

function syncServersCacheFromLoader() {
  for (const name of Object.keys(servers)) {
    delete servers[name];
  }

  for (const serverConfig of configLoader.getAllServers()) {
    servers[serverConfig.name] = toRuntimeServerConfig(serverConfig);
  }
}

function snapshotServerState() {
  return {
...
function restoreServerState(snapshot) {
  configLoader.servers = new Map(snapshot.loaderServers);
  configLoader.configSource = snapshot.configSource;

  for (const name of Object.keys(servers)) {
    delete servers[name];
  }
  Object.assign(servers, snapshot.serverCache);
}
...
try {
  ...
  for (const [name, config] of loadedServers) {
    servers[name] = toRuntimeServerConfig(config);
  }
...
const dashboardService = createDashboardService({
  ...
  snapshotState: snapshotServerState,
  restoreState: restoreServerState,
  beforeEdit: (oldNormalized, newNormalized) => {
    closeConnection(oldNormalized);
    if (oldNormalized !== newNormalized) {
      closeConnection(newNormalized);
      dashboardTestResults.delete(oldNormalized);
    }
  },
  beforeDelete: (normalizedName) => {
    closeConnection(normalizedName);
  },
  afterSave: syncServersCacheFromLoader,
  afterRestore: syncServersCacheFromLoader,
  testResults: dashboardTestResults
});
...
registerToolConditional(
  ...
  async ({ server, command, password, cwd, timeout = 30000 }) => {
    try {
      ...
      const baseCommand = command.replace(/^sudo\s+/, '').trim();

      let fullCommand = `sudo -S -p '' ${baseCommand}`;
...
// Start the server
async function main() {
  const dashboardEnabled = process.env.SSH_DASHBOARD_EMBEDDED === 'true';
  if (dashboardEnabled) {
    startDashboardServer({
      host: process.env.SSH_DASHBOARD_HOST || '127.0.0.1',
